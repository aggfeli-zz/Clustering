...
{ 
    ...
    {  
        Hashtable **PointersToHashtable = CreateHash(L, NumofHashFUnctions , 0); //Construct L HashTables
            for(int j = 0; j < N; j++)
            {
...
void QuerySearch(Cluster **cluster, Hashtable **PointersToHashtable, int L, int k, string method, int counter, int NumofHashFUnctions)
{
    ...
    if (method == "@metric_space hamming") radius = min_dist / 2;
    ...
    for(int z = 0; z < 5; z++)
    {
        for(int j = 0; j < k; j++)
        {
            //cout << "   " << j << "     " << cluster[j]->getCentroid() << endl;
            min = 1000; 
            ...
            for (int i = 0; i < 1; i++)   //Search  neighbour within radius
            {
